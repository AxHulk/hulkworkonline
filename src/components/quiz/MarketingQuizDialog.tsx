...
const goalOptions: { v: string; label: string }[] = [
...
const teamOptions = [
  { v: "solo", label: "Я один(а)" },
  { v: "2_3", label: "2–3 человека" },
  { v: "4_10", label: "4–10 человек" },
  { v: "10plus", label: "10+ человек" },
];
...
const contactSchemas: Record<Channel, z.ZodTypeAny> = {
  ...
  whatsapp: z.string().trim().min(7).max(30).refine(
    (v) => /^\+?\d[\d\s\-()]{7,20}$/.test(v), "Номер в формате +7 999 123-45-67"
  ),
...
const findLabel = (opts: { v: string; label: string }[], v: string) =>
  opts.find((o) => o.v === v)?.label || "—";
...
const MarketingQuizDialog = () => {
  ...
  const validateStep = (): string | null => {
    switch (step) {
      case 1:
        if (!state.goal) return "Выберите главную цель.";
        if (state.goal === "other" && state.goalOther.trim().length < 3) return "Опишите вашу цель.";
        return null;
      case 2: return state.result6_12 ? null : "Выберите ожидаемый результат.";
      case 3:
        if (!state.benchmarks) return "Выберите вариант.";
        if (state.benchmarks === "local" && state.benchmarksLocal.trim().length < 2)
          return "Укажите имена локальных конкурентов.";
        return null;
      case 4: return state.has.length === 0 ? "Выберите хотя бы один вариант." : null;
      case 5: return state.clientsPerMonth ? null : "Выберите вариант.";
      case 6: return state.avgCheck ? null : "Выберите вариант.";
      case 7: return state.margin ? null : "Выберите вариант.";
      case 8: return state.scaleReady ? null : "Выберите вариант.";
      case 9: return state.teamSize ? null : "Выберите вариант.";
      case 10: return state.salesHandler ? null : "Выберите вариант.";
      case 11: return state.budget ? null : "Выберите бюджет.";
      case 12: return state.payback ? null : "Выберите вариант.";
      case 13: return state.budgetType ? null : "Выберите вариант.";
      case 14: return state.cooperation ? null : "Выберите вариант.";
      case 15: return state.meetingFreq ? null : "Выберите вариант.";
      case 16: return state.commsChannel ? null : "Выберите вариант.";
      case 17:
        if (state.critical.length === 0) return "Выберите хотя бы один вариант.";
        if (state.critical.length > 3) return "Не больше 3 вариантов.";
        return null;
      case 18: return state.channels.length === 0 ? "Выберите хотя бы один канал." : null;
      case 19:
        if (!state.constraints) return "Выберите вариант.";
        if (state.constraints === "other" && state.constraintsOther.trim().length < 3)
          return "Опишите ограничение.";
        return null;
      case 20: return state.deadline ? null : "Выберите вариант.";
      case 21: return state.prevAgency ? null : "Выберите вариант.";
      case 22: {
        ...
        if (!consent) { setConsentError(true); return "Необходимо согласие на обработку персональных данных."; }
        return null;
      }
      default: return null;
    }
  ...
  const submit = async () => {
    ...
    try {
      const answersHuman = {
        goal: state.goal === "other" ? `Другое: ${state.goalOther}` : findLabel(goalOptions, state.goal),
        result6_12: findLabel(result6_12Options, state.result6_12),
        benchmarks: state.benchmarks === "local"
          ? `Локальные конкуренты: ${state.benchmarksLocal}`
      ...
      const { error } = await (supabase.from("quiz_submissions") as any).insert({
        answers: { ...state, humanReadable: answersHuman, track: "marketing", source },
  ...
  return (
    <Dialog open={isOpen} onOpenChange={(o) => { if (!o) handleClose(); }}>
      <DialogContent className="max-h-[92vh] max-w-2xl overflow-y-auto p-0 sm:rounded-2xl">
        <DialogTitle className="sr-only">Опросник по маркетингу и SMM</DialogTitle>
        <DialogDescription className="sr-only">
          Ответьте на 22 коротких вопроса — менеджер свяжется с вами в течение 12 часов.
        </DialogDescription>
                ...
                <StepWrap title="Как с вами связаться?">
                  <div className="space-y-3">
                    <div className="space-y-2">
                      ...
                      <Input value={state.contactName} onChange={(e) => update("contactName", e.target.value)}
                        placeholder="Имя" maxLength={80} />
                    </div>
                    <div className="space-y-2">
            ...
            <div className="sticky bottom-0 z-10 border-t bg-background/95 px-6 py-4 backdrop-blur">
              ...
              <div className="flex items-center justify-between gap-3">
                <Button type="button" variant="ghost" onClick={goPrev}
                  disabled={step === 1 || submitting} className="gap-2">
...
const CheckList = ({ options, values, onToggle }: {
  options: { v: string; label: string }[]; values: string[]; onToggle: (v: string) => void;
}) => (
  <div className="grid gap-2">
    {options.map((o) => {
...
const FinalScreen = ({ onClose, name }: { onClose: () => void; name: string }) => (
  <div className="px-6 py-10 text-center">
    ...
    <p className="mt-3 text-muted-foreground leading-relaxed">
      Маркетинг — это всегда индивидуальная стратегия: точные условия и каналы зависят
      от вашей ниши, ресурсов и целей. Поэтому, чтобы не давать «универсальных» ответов,
      мы подготовим персональное предложение под ваш бизнес.
    </p>
    <p className="mt-3 text-muted-foreground leading-relaxed">
      В течение ближайших <strong>12 часов</strong> с вами свяжется персональный менеджер
      и предложит оптимальный сценарий продвижения.
    </p>
    <div className="mt-6 rounded-2xl border bg-gradient-to-br from-primary/10 to-accent/30 p-5 text-left text-sm text-muted-foreground">
      <p>
        <strong>Что дальше:</strong> мы изучим ваш проект, конкурентов и аудиторию, проведём
        предварительный аудит и придём с прозрачным планом — без скрытых условий.
      </p>
    </div>
    <Button onClick={onClose} className="mt-6 w-full font-heading" size="lg">Закрыть</Button>
  </div>
);
