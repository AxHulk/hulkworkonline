...
import totem1 from "@/assets/case-totem-ufa-1.png";
import totem2 from "@/assets/case-totem-ufa-2.png";
import totem3 from "@/assets/case-totem-ufa-3.png";
...
const CITY_CASES: Partial<Record<`${ServiceKey}_${CityKey}`, CityCase>> = {
  ...
  "web-development_ufa": {
    title: "Интернет-магазин женской одежды TOTEM в Уфе",
    client: "TOTEM, бренд женской одежды, Уфа",
    summary:
      "Сайт представляет собой высокотехнологичную платформу электронной коммерции, разработанную для производителя женской одежды. Особой гордостью в данном проекте является создание абсолютно бесшовного покупательского опыта. Удалось разработать продукт, в котором визуальная эстетика не конфликтует с технической функциональностью. Сложные банковские интеграции, работа с базами данных и системы фильтрации товаров работают мгновенно и незаметно для пользователя. Интерфейс интернет-магазина получился современным, интуитивно понятным и полностью соответствующим высоким стандартам индустрии моды.",
    highlights: [
      "Бесшовный покупательский опыт: визуальная эстетика и техническая функциональность работают как единое целое",
      "Мгновенные банковские интеграции и онлайн-оплата без перезагрузок и лишних шагов",
      "Производительная работа с большой базой товаров и быстрые фильтры по размеру, цвету и категориям",
      "Современный fashion-интерфейс уровня индустрии моды — минимализм, крупная типографика, акцент на фото",
    ],
    images: [
      { src: totem1, alt: "Lookbook бренда TOTEM — главный экран интернет-магазина женской одежды в Уфе" },
      { src: totem2, alt: "Каталог TOTEM с разделами «Костюмы и комплекты» и «Пиджаки и жакеты»" },
      { src: totem3, alt: "Карточка товара TOTEM — демисезонная куртка с описанием, составом и обмерами" },
    ],
  },
};
